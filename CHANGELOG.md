...
- **MCP write concurrency and selected-palace consistency.** Mutating MCP tools now acquire the same per-palace write lock as `mempalace mine`, so drawer, diary, and knowledge-graph writes serialize with mines and with other MCP writers for the same palace. MCP write lock timeouts return structured tool results instead of breaking JSON-RPC, and read-only tools remain unlocked. Palace path selection now consistently expands `~` for env, config, default, and `--palace`; MCP knowledge graphs live under the effective selected palace, with one-time copy migration from the legacy KG path when needed. WAL entries now include the effective `palace_path` while preserving content redaction.
